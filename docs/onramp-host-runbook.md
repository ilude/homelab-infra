...
- BWS `HOMELAB_ANSIBLE_INVENTORY`: BWS project/API server configuration, source app-definition checksum, digest-pinned images for all six containers, `onclave_onramp_authorized_keys` (for example, `ssh-ed25519 <public-key> operator@example.invalid`), and the adopted PostgreSQL database and user inputs, `onclave_onramp_postgres_database` and `onclave_onramp_postgres_user`. Their physical values remain `menos`.
...
The controller receives only `BITWARDEN_ACCESS_KEY` from the operator environment and resolves the Onclave secrets before running the role. The bootstrap credential is not copied to the managed host. The role verifies the source app-definition checksum and maps it to six native rootless Podman Quadlet containers plus one private network. Initial cutover starts the complete target. Later applies render the same native contracts, update Caddy, and restart only the unit whose environment, image, or container template changed. Authorized-key changes restart only core. Network or target-membership topology changes restart the complete target once. The rollout report distinguishes requested restart actions from systemd's `NRestarts` automatic-restart counter. RabbitMQ is private to that network, the internal service URL remains `rabbitmq:5672`, and only the core API is published on loopback behind shared Caddy. RabbitMQ/core data stays below the service deployment directory; the adopted PostgreSQL and Ollama directories remain in place. The role discovers and reuses the existing SearXNG volumes on cutover and steady-state applies. Clients outside the onramp host require an approved local transport or tunnel.

The core applies the additive, idempotent `20260722_job_durability` PostgreSQL migration during vault construction, before opening its HTTP listener. The role does not run a second migration command. It orders healthy PostgreSQL before core, then requires post-start readiness before accepting the rollout. `/live` is the process-only container liveness probe. `/health` is diagnostic and may safely report `503 degraded` for a transcript outage without restarting core. `/ready` checks the configured dependencies and selected providers with finite redacted values. `/metrics` exposes the fixed public Prometheus families; validation does not generate synthetic work merely to create samples.

Before changing Onclave state, create the device-local recovery archive with `scripts/service-state.sh backup onclave_onramp`. It covers the deployment, Caddy snippet, and adopted PostgreSQL data. Back up SeaweedFS application objects separately with `infra/ansible/playbooks/seaweedfs-object-backup.yml`, using a quiescence marker and a private archive under `values/service-backups/onclave_onramp/`; run its `restore-test` operation before relying on that archive. Restore host-local Onclave state with `scripts/service-state.sh restore onclave_onramp latest`, or restore a reported history basename. Device-local history does not protect against loss of the Onramp host. Ollama data is rebuildable and is not archived.
...
5. Verify rootless `podman info`, systemd user services, and deployment directory ownership.
6. If app services such as `infisical_onramp` or `onclave_onramp` are enabled, let this repo deploy them through Ansible on the onramp host. Onclave runs as native Quadlet units; its SearXNG container remains internal to that workload.
...
9. For Onclave, verify process liveness, source revision, diagnostic structure, dependency readiness, Prometheus family declarations, and signed API access. Diagnostic health may be degraded while readiness remains usable. Verify AMQP only through an approved local transport or tunnel, not a direct LAN connection.
