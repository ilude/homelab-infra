...
The controller receives only `BITWARDEN_ACCESS_KEY` from the operator environment and resolves the Onclave secrets before running the role. The bootstrap credential is not copied to the managed host. The role verifies the source app-definition checksum and maps it to six native rootless Podman Quadlet containers plus one private network. RabbitMQ is private to that network, the internal service URL remains `rabbitmq:5672`, and only the core API is published on loopback behind shared Caddy. RabbitMQ/core data stays below the service deployment directory; the adopted PostgreSQL and Ollama directories remain in place. The role discovers and reuses the existing SearXNG volumes during cutover. Clients outside the onramp host require an approved local transport or tunnel.
