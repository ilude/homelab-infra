...
The Onclave source repository publishes a reusable app definition and immutable image contract. `onclave_onramp` verifies that definition, maps its six retained services to native rootless Podman Quadlet units, applies consumer-owned networking and storage bindings, and keeps source and image references digest-pinned. Do not replace this path with mutable images, local source builds, or an independently maintained Compose deployment.
...
Onclave exposes its API through the onramp host's shared Caddy instance. RabbitMQ has no host-published AMQP or management listener. Native Quadlet containers continue to use `rabbitmq:5672` on their private Podman network; clients outside the onramp host require an approved local transport or tunnel. PostgreSQL, Ollama, SearXNG, and Docling remain internal to the workload network. Onclave uses the separately managed SeaweedFS `menos` bucket through internal HTTPS. `scripts/service-state.sh backup onclave_onramp` quiesces and restarts the native target while covering the deployment, Caddy snippet, and adopted PostgreSQL data; `seaweedfs-object-backup.yml` creates and restore-tests the separate application-object archive. Rebuildable Ollama data is excluded.
