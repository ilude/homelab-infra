...
class ServiceStateCliTests(unittest.TestCase):
    ...
    def test_registry_state_capability_drives_list_backup_all_and_validation(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            backed_up = run_script(
                script, "backup", "all", cwd=root, env={"CAPTURE_FILE": str(capture)}
            )
            ...
            self.assertIn(
                "service_state_service='eligible'", capture.read_text(encoding="utf-8")
            )

            rejected = run_script(script, "backup", "ineligible", cwd=root)
            self.assertEqual(2, rejected.returncode)
            self.assertIn(
                "Unsupported service-state target: ineligible", rejected.stderr
            )
    ...
    def test_windows_backup_acl_hardening_is_fail_closed(self) -> None:
        ...
        playbook = (
            REPO / "infra/ansible/playbooks/service-state-backup.yml"
