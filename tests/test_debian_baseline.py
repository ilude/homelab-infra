...
class DebianBaselineTests(unittest.TestCase):
    def test_lxc_template_uses_the_verified_debian_13_download_with_checksum(
        self,
    ) -> None:
        ...
        self.assertIn(
            "checksum_algorithm  = var.debian_13_lxc_template_checksum_algorithm", text
        )
        moves = (REPO / "infra" / "opentofu" / "services.tf").read_text(
            encoding="utf-8"
        )
    ...
    def test_every_lxc_service_uses_the_debian_13_template(self) -> None:
        ...
        for name, module_name in expected_modules.items():
            ...
            self.assertIn(
                "proxmox_download_file.debian_13_lxc_template[0].id", text, name
            )
        ...
        module_text = (root / "modules" / "debian-lxc" / "main.tf").read_text(
            encoding="utf-8"
        )
        self.assertEqual(
            len(
                re.findall(
                    r'resource\s+"proxmox_virtual_environment_container"', module_text
                )
            ),
            1,
        )
    ...
    def test_onramp_host_keeps_its_separate_verified_debian_13_image(self) -> None:
        text = (REPO / "infra" / "opentofu" / "onramp-host.tf").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn(
            "checksum_algorithm  = var.onramp_host_image_checksum_algorithm", text
        )
