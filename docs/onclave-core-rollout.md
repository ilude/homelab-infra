...
The Ansible playbook persists the selected core image pin in the native
`onclave-core.container` Quadlet and the source revision in `env/core.env`, then
pulls the immutable image and restarts only `onclave-core.service`. It does not
restart `onclave-onramp.target`, RabbitMQ, PostgreSQL, SeaweedFS, Ollama,
SearXNG, Docling, or Caddy. It fails immediately if the core container exits or
reports a restart, then waits for HTTP `/health` to return `status=ok`, the
requested `git_sha`, and `broker.connected=true`. Failures include at most 200
lines of redacted core logs.
