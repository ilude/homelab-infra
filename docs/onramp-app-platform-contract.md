...
Onclave exposes its API through the onramp host's shared Caddy instance. RabbitMQ AMQP and management remain loopback-only with no LAN or public listener. Internal Compose services continue to use `rabbitmq:5672`; clients outside the onramp host require an approved local transport or tunnel. PostgreSQL, Ollama, SearXNG, and Docling remain internal to the workload network. Onclave uses the separately managed SeaweedFS `menos` bucket through internal HTTPS. `scripts/service-state.sh backup onclave_onramp` covers the deployment, Caddy snippet, and adopted PostgreSQL data; `seaweedfs-object-backup.yml` creates and restore-tests the separate application-object archive. Rebuildable Ollama data is excluded.
